...
> Update 2021-03-21: A less-than-ideal but much easier fix is to build xl2tpd from source with one problematic line [commented out](https://github.com/xelerance/xl2tpd/issues/136#issuecomment-365761037). This allows the use of Libreswan and NetworkManager, although on Arch Linux I also had to `sudo mkdir -p /var/lib/ipsec/nss` since that directory wasn't created during Libreswan installation (?!)
